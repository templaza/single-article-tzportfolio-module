...
//$params     = $this -> params;
if(isset($params) && $params -> get('show_about_author', 1) && isset($authorId)){
    JLoader::import('user.profile.helpers.profile', COM_TZ_PORTFOLIO_PLUS_ADDON_PATH);

    if($author = PlgTZ_Portfolio_PlusUserProfileHelper::getAuthorAbout($authorId)){
        $lang   = JFactory::getLanguage();
        $doc    = JFactory::getDocument();

        $lang -> load('com_tz_portfolio_plus');
        $doc -> addStyleSheet(TZ_Portfolio_PlusUri::base(true).'/addons/user/profile/css/style.css',
                ...
                <?php if($params -> get('show_social_link_user', 1) && (!empty($author -> twitter)
                    ...
                    <ul class="list-inline social small">
                        <?php if($author -> twitter){?>
                            <li class="list-inline-item">
                                <a href="<?php echo $author -> twitter;
                                ?>" target="_blank"><span class="tpb tp-twitter"></span></a>
                            </li>
                        <?php } ?>
                        <?php if($author -> facebook){?>
                            <li class="list-inline-item">
                                <a href="<?php echo $author -> facebook;
                                ?>" target="_blank"><span class="tpb tp-facebook-square"></span></a>
                            </li>
                        <?php } ?>
                        <?php if($author -> instagram){?>
                            <li class="list-inline-item">
                                <a href="<?php echo $author -> instagram
                                ; ?>" target="_blank"><span class="tpb tp-instagram"></span></a>
                            </li>
                        <?php } ?>
                        <?php if($author -> googleplus){?>
                            <li class="list-inline-item">
                                <a href="<?php echo $author -> googleplus;
                                ?>" target="_blank"><span class="tpb tp-google-plus-g"></span></a>
                            </li>
                        <?php } ?>
                    </ul>
                <?php } ?>
